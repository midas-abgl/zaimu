import type { Account, Category, Debt, Loan, Salary, Subscription, Transaction } from "@/lib/api";
...
export const localAccounts = {
	bulkPut: (items: Array<{ data: Account; localId: string; syncedAt?: number }>) =>
	...
	getModifiedSince: (since: number) => getModifiedSince<Account>(STORES.accounts, since),
	put: (data: Account, id?: string) => put(STORES.accounts, data, id),
};
