...
function DebtsPage() {
	...
	const { data: debtsData, isLoading } = useQuery({
		enabled: !!user?.id,
		queryFn: () => api.getDebts({ userId: user?.id }),
		queryKey: ["debts", user?.id],
	});

	const { data: summary } = useQuery({
		enabled: !!user?.id,
		queryFn: () => api.getDebtsSummary(user!.id),
		queryKey: ["debts-summary", user?.id],
	});

	const createMutation = useMutation({
		mutationFn: (data: Parameters<typeof api.createDebt>[0]) => api.createDebt(data),
		onSuccess: () => {
	...
	const markPaidMutation = useMutation({
		mutationFn: ({ id }: { id: string }) =>
			api.updateDebt(id, { isPaid: true, paidDate: new Date().toISOString() }),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["debts"] });
			queryClient.invalidateQueries({ queryKey: ["debts-summary"] });
		},
	...
	const totals = debtsData?.totals || { iOwe: 0, net: 0, owedToMe: 0 };

	const owedToMeDebts = debts.filter(d => d.isOwedToMe && !d.isPaid);
	const iOweDebts = debts.filter(d => !d.isOwedToMe && !d.isPaid);
	const settledDebts = debts.filter(d => d.isPaid);

	const getDisplayDebts = () => {
		switch (activeTab) {
			case "owedToMe":
				return owedToMeDebts;
			case "iOwe":
				return iOweDebts;
			case "settled":
				return settledDebts;
	...
	const displayDebts = getDisplayDebts();
			...
			>
				...
				<div className="relative z-10">
					<div className="mb-6 flex items-center justify-between">
						<h1 className="font-bold text-2xl text-white">Debts</h1>
						<button
				...
				<div className="fixed inset-0 z-50 flex items-end justify-center">
					...
					<div className="relative max-h-[90vh] w-full max-w-lg animate-slide-up overflow-y-auto rounded-t-3xl bg-white p-6">
						...
						<div className="mb-6 flex items-center justify-between pt-2">
							<h2 className="font-bold text-foreground text-xl">New Debt</h2>
							<button
						...
						{/* Person Name */}
						<div className="mb-4">
							<span className="mb-2 block font-medium text-foreground-muted text-sm">Person</span>
							<input
								className="input"
						...
						<div className="mb-4 grid grid-cols-2 gap-3">
							...
							<div>
								<span className="mb-2 block font-medium text-foreground-muted text-sm">Due Date</span>
								<input
									className="input"
